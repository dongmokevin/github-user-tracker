...
export default function Profile() {
  ...
  if (user) {
    ...
    let hire;
    if (user.hireable) {
      hire = <div className="badge bg-success">Available for hire</div>;
    } else {
      hire = <div className="badge bg-danger">Unavailable for hire</div>;
    }
    return (
      <div>
        <Link
        ...
        <div className="card w-75 mb-2">
          <div className="card-body d-flex flex-row bg-light">
            ...
            <div>
              ...
              <ul className="list-group list-group-flush">{repoList}</ul>
            </div>
          </div>
        </div>
      </div>
